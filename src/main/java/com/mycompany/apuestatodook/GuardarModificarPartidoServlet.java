...
@WebServlet(name = "GuardarModificarPartido", urlPatterns = {"/GuardarModificarPartido"})
public class GuardarModificarPartidoServlet extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
       ...
       /*
        if (local.equals("") || visitante.equals("") || fecha.equals("")) {
            ...
            //request.getRequestDispatcher("/ApuestaTodook/ModificarPartido?id="+id).forward(request, response);
            request.setAttribute("id", id);
            //request.getRequestDispatcher("/ModificarPartido").forward(request, response);
        }
        ...
        if (!hayError) {
            ...
            partidoDAO.updatePartidoPorId(id, local, visitante, fecha);
            request.getRequestDispatcher("/WEB-INF/jsp/partidoModificadoOK.jsp").forward(request, response);
        }
        else
        {
            request.setAttribute("hayError", true);
            request.setAttribute("mensajeError", "Se deben completar todos los campos.");
            request.setAttribute("id", id);
            //request.getRequestDispatcher("/WEB-INF/jsp/partidoModificadoError.jsp").forward(request, response);
